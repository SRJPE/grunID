...
#' @param online_mode either TRUE or FALSE - if TRUE, you must pass a value for plate_run_id. If
#' FALSE, you do not need to pass a plate_run_id.
...
process_sherlock <- function(filepath,
                             ...
                             layout_type = c("split_plate_early_late", "split_plate_spring_winter", "triplicate", "single_assay"),
                             single_assay_type = c("ots28 early", "ots28 late", "ots16 spring", "ots16 winter", NULL),
                             plate_run_id, plate_size = c(96, 384),
                             online_mode = TRUE/FALSE) {

  if(online_mode) {
    if(missing(plate_run_id)) {
      stop("If you are running this function in online mode, you must provide a plate_run_id.")
    }
    ...
    sample_details <- process_well_sample_details(filepath = filepath,
                                                  ...
                                                  single_assay_type = single_assay_type,
  ...
  raw_assay_results <- process_raw_assay_results(filepath, ranges = cell_ranges, plate_size, layout, online_mode = !!online_mode)
...
process_raw_assay_results <- function(filepath, ranges, plate_size, layout, online_mode) {
  # raw fluorescence ----
...
process_well_sample_details <- function(filepath,
  ...
  if(online_mode) {
    if(missing(plate_run_id)) {
      stop("If you are running this function in online mode, you must provide a plate run ID.")
    }
    ...
    # single assay
      ...
      assay_id <- dplyr::case_when(single_assay_type == "ots28 early" ~ 1,
