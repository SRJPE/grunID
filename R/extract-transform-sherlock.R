...
#' @param plate_run_id plate run identifier generated by running `add_plate_run()`. If this argument
#' is missing, this function will run in offline mode, i.e. will not interact with the database.
...
process_sherlock <- function(filepath,
  ...
  raw_assay_results <- process_raw_assay_results(filepath, ranges = cell_ranges, plate_size, layout, has_background_fluorescence = has_blk_entries, offline_mode = run_offline_mode)
...
process_raw_assay_results <- function(filepath, ranges, plate_size, layout, has_background_fluorescence, offline_mode) {
  # raw fluorescence ----
  ...
  if(!offline_mode) {
    ...
    raw_assay_results <- raw_fluorescence |>
      dplyr::left_join(background_fluorescence) |>
      dplyr::select(sample_id, sample_type_id, assay_id, raw_fluorescence = fluorescence,
                    background_value = background_fluorescence,
                    time = Time, well_location = location) |>
      dplyr::filter(!is.na(sample_id))
  }
}
...
#' Get Sample Details Offline
#' @description `process_well_sample_details_offline` reads in the plate run layout so sample identifiers
#' can be mapped to the output of the Synergy H1 reader.
#' @param filepath Synergy H1 reader output excel file with corresponding plate run layout
#' in a sheet titled "Plate Map"
#' @param sample_type either `mucus` or `fin_clip`
...
#' `single_assay_ots28_early`, `single_assay_ots28_late`, `single_assay_ots16_spring`, or
#' `single_assay_ots16_winter`
#' @details this function is called when sherlock results are being processed offline, i.e. without
#' interacting with the database. There is an alternative version of this function that interacts with the
#' database: `process_well_sample_details`. The output of this function will be used as the `sample_details`
#' argument in `process_sherlock()` i.e. it will be used to connect the generic sample identifiers produced
#' by the Synergy H1 reader to sample identifiers in the database. If the `layout_type` argument is
#' `split_plate`, it divide the plate into columns 1:12 and 13:24 and assigns assays based on the
#' following logic:
#' * `split_plate_early_late`: columns 1:12 are OTS 28 Early and columns 13:24 are OTS 28 Late
...
#' * rows D, H, L, and P are OTS 16 Winter
#'
#' If the `layout_type` is one of `single_assay_ots28_early`, `single_assay_ots28_late`, `single_assay_ots16_spring`,
#' `single_assay_ots16_winter`, all samples processed in that plate run will be assigned that assay.
#'
#' @returns a table to be passed to `process_sherlock()`:
#' ## plate_layout
...
#' @export
#' @md
process_well_sample_details_offline <- function(filepath,
                                                sample_type = c("mucus", "fin clip"),
                                                layout_type = c("split_plate_early_late", "split_plate_spring_winter", "triplicate",
                                                                "single_assay_ots28_early", "single_assay_ots28_late",
                                                                "single_assay_ots16_spring", "single_assay_ots16_winter")) {
