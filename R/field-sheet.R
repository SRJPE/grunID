...
create_field_sheet <- function(wb, field_sheet_sample_plan,
  ...
  addStyle(wb, sheet = sheet_name, style = col_style, rows = row_range, cols = row_range)
...
data <- tibble(
  ...
  `Sample #` = 1:120,
  `Sample ID` = paste0("BTC22_3_A_", 1:120),
  Date = "",
  Time = "",
  `FL (mm)` = "",
  `Field Run ID` = "",
...
wb <- createWorkbook()
wb <- create_field_sheet(wb = wb, sample_event_number = 3,
                         field_sheet_sample_plan = data,
                         first_sample_date = first_sample_date,
                         sample_location = "Battle Creek", "BTC")
