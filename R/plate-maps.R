...
make_single_assay_layout <- function(data, output_dir, season_filter, events_name,
  ...
  if (length(layouts_list) >= 1) single_assay[seq(1, 16, by = 2), seq(1, 24, by = 2)] <- as.matrix(layouts_list[[1]])
  if (length(layouts_list) >= 2) single_assay[seq(1, 16, by = 2), seq(2, 24, by = 2)] <- as.matrix(layouts_list[[2]])
  if (length(layouts_list) >= 3) single_assay[seq(2, 16, by = 2), seq(1, 24, by = 2)] <- as.matrix(layouts_list[[3]])
  if (length(layouts_list) >= 4) single_assay[seq(2, 16, by = 2), seq(2, 24, by = 2)] <- as.matrix(layouts_list[[4]])
