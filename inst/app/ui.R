...
navbarMenu("Add Data",
           ...
           # Check-in Samples --------------------------------------------------------
           # TODO: add preview of new samples to be checked in (as table)
           # (sample_ids that were part of the check in file that was uploaded)
           # This should occur BEFORE user hits submit - "preview button? or if we have access without"
           # After hitting submit, if GrunId created NEW samples; show these as a DT;
           #   this should be included as a function already ("check_in_samples_v2.R) or something
           tabPanel(
             "Check-in Samples",
             ...
             fluidRow(
               column(6,
                      h4("Preview Uploaded Table"),
                      tableOutput("check_in_preview")
               ),
               column(6,
                      h4("New Samples"),
                      tableOutput("check_in_file_submission")
               )
             )
           ),
