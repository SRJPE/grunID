...
The function `process_well_sample_details()` takes in an excel table with a sheet titled `Plate Map`. The
`Plate Map` sheet should contain the well layout locations with JPE sample IDs *(e.g., A1 and F1722_3_A_1)*.
We can then extract the sample details and format them into the desired dataframe using:

```{r, eval = FALSE, cache = TRUE}
sample_details <- process_well_sample_details(filepath = "path_to/SynergyH1_data.xlsx",
                                                         sample_type = "mucus",
                                                         assay_type = "Ots28_Early1",
                                                         plate_run_id = plate_run_4_7_early_id)
```

The sample_details is now a data frame that looks like:
```{eval=FALSE}
...
plate_run_uid <- grunID::add_plate_run(con, protocol_id = 1, genetic_method_id = 1,
                                       laboratory_id = 1, lab_work_performed_by = "Yogi Bear")
