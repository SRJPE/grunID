...
There are two ways you can query for sample status: 

1) find the sample status of a specific sample using `get_sample_status` 
2) find all samples requiring further analysis using `get_samples_needing_action`. This will produce a named list organized into the following categories: 
...
```{r, eval = FALSE, cache = TRUE}
# find the status of a specific sample "Sample_ID"
grunID::get_sample_status(con, "Sample_ID", full_history = FALSE)
...
sample_details <- grunID::process_well_sample_details(filepath = "templates/sherlock_results_template.xlsx",
                                                      sample_type = "mucus",
                                                      layout_type = "split_plate_early_late",
                                                      plate_run_id = plate_run_id)
